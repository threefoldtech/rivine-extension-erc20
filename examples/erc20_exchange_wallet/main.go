...
import (
	...
	"github.com/threefoldtech/rivine-extension-erc20/api/bridge"
)
...
type demoExchange struct {
	contract *bridge.BridgeContract
}
...
func (cmd *Commands) Root(_ *cobra.Command, args []string) error {
	...
	contract, err := bridge.NewBridgeContract(strings.ToLower(cmd.EthNetworkName), cmd.EthBootNodes, cmd.ContractAddress, int(cmd.EthPort), cmd.accJSON, cmd.accPass, cmd.RootPersistentDir, closeChan)
	if err != nil {
